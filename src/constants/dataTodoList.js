export const dataTodoList = [
	{
		id: 1,
		name: 'constants.js',
		language: 'javascript', // Изменено с 'js' на 'javascript' для совместимости с Prism
...
export const PAST_CLASS = 'bg-black text-white cursor-pointer';`
	},
	{
		...
		language: 'javascript',
		content: `// СЧЕТЧИК ЗАЛОВ, В КОТОРЫХ ИДЕТ ВЫБРАННЫЙ ФИЛЬМ
export const counterHalls = (elements, item) => {
  ...
  return counter.length === elements.length ? [999] : counter;
	...
	},
	{
		...
		language: 'javascript',
		content: `import { PETERBURG_MOVIE_THEATERS } from "../constants/cinemaСhain/cities/peterburgConstants/peterburgConstants";
...
const cinemaHalls = (data) => {
  if (!data || !Array.isArray(data.halls)) return [];
